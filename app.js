...
function drawEnvelope(state) {
  ...
  for (const h of hist) {
    for (const id of ids) {
      const v = h.capability[id];
      if (v === undefined) continue;
      if (v > max) max = v;
      if (v < min) min = v;
    }
  }
  const span = Math.max(max - min, 1) * 1.12;
  const base = min - Math.max(max - min, 1) * 0.06;
  ...
  for (let g = 0; g <= 4; g++) {
    ...
    ctx.stroke();
    ctx.fillText(val.toFixed(0), 6 * dpr, yy + 3 * dpr);
  }
