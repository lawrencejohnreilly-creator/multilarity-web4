...
const { LOCUS_DIMENSIONS, logSumExp, logShares } = require('./ecology');
...
function lineageMonitor(eco) {
  ...
  // Shares come from log-sum-exp rather than a linear total. LD is an
  // inverse-HHI over shares and is scale-invariant, so it is unaffected
  // by how large the frontier has grown — which is the point.
  const pShares = logShares(act.map((p) => p.logCap));
  const shares = new Map();
  act.forEach((p, i) => {
    const key = p.dims.modelLineage;
    shares.set(key, (shares.get(key) || 0) + pShares[i]);
  });
  let hhi = 0;
  for (const v of shares.values()) hhi += v * v;
  const ld = shares.size ? 1 / hhi : 0;

  // Two different ceilings bound this number and only one of them is a wall.
  //
  // Without creating a new lineage, inverse-HHI cannot exceed the count of
  // lineages present — that is ldCeilingNow, and remediation can raise it.
  //
  // What remediation cannot raise: a participant is atomic and cannot be
  // split across lineages, so the largest lineage is never smaller than the
  // largest participant. HHI is therefore at least pMax squared and LD is at
  // most its reciprocal, whatever the lineage count. LD is bound by
  // concentration, not by labelling. When that bound sits under the
  // threshold, separating lineages is pushing on a variable that cannot
  // close the gap and the finding must say so rather than run forever.
  const distinctLineages = shares.size;
  const ldCeilingNow = distinctLineages;
  const pMax = act.length ? Math.max(...pShares) : 1;
  const ldHardCeiling = pMax > 0 ? 1 / (pMax * pMax) : 0;
  const ldReachable = ldHardCeiling >= LD_FAIL;
  // The concentration a participant must fall under for the threshold to be
  // attainable at all, published so the remediation target is unambiguous.
  const pMaxRequired = 1 / Math.sqrt(LD_FAIL);
  ...
  return {
    ...
    distinctLineages,
    ldCeilingNow,
    ldHardCeiling: Number(ldHardCeiling.toFixed(3)),
    ldReachable,
    pMax: Number(pMax.toFixed(3)),
    pMaxRequired: Number(pMaxRequired.toFixed(3)),
    concentrationBound: ldHardCeiling < ldCeilingNow,
    dominantLineage,
    dominantMember,
...
function indexer(eco, parts) {
  ...
  // Concentration over loci, taken in log space. A share is a ratio and
  // stays exact however far apart the participants have grown.
  const locusShares = logShares(loci.map((l) => l.logCap));
  const ccr = locusShares.length ? Math.max(...locusShares) : 1;
  ...
  const ldTermCeiling = Number(
    Math.max(0, Math.min(1, (lineage.ldHardCeiling - 1) / (n - 1))).toFixed(4)
  );
  ...
  return {
    ...
    ld: lineage.ld,
    ldCeilingNow: lineage.ldCeilingNow,
    ldHardCeiling: lineage.ldHardCeiling,
    ldTermCeiling,
    distinctLineages: lineage.distinctLineages,
    pMax: lineage.pMax,
    pMaxRequired: lineage.pMaxRequired,
    ldConcentrationBound: lineage.concentrationBound,
...
function pathologyWatch(eco, parts) {
  ...
  if (lineagesShort || halfLifeShort) {
    ...
    out.push({
      ...
      blockedReason: `MC-2 cannot be satisfied by separating lineages: largest participant holds ${(lineage.pMax * 100).toFixed(1)}% of capability, capping LD at ${lineage.ldHardCeiling} against a threshold of ${LD_FAIL}. LD is bound by concentration, not by lineage count — no participant may exceed ${(lineage.pMaxRequired * 100).toFixed(1)}% for this condition to be attainable.`,
    });
  }
...
// Improvement is directional. For most probes a larger number is closer to
// passing; for interface share a smaller one is. Needed because the stall
// test asks whether the correction got nearer the threshold, not whether the
// number twitched.
const PROBE_DIRECTION = {
  'separate-lineage': 1,
  'restore-lineage-independence': 1,
  'dual-custody': 1,
  'negotiate-interface': -1,
  'exit-drill': 1,
};

const PROBE_TARGET = {
  'separate-lineage': LD_FAIL,
  'negotiate-interface': 0.45,
  'exit-drill': 0.6,
};
...
function sentinel(eco, pathologies, mode, queue, parts) {
  ...
  for (const p of pathologies) {
    ...
    // Movement is not progress. A correction that nudges the number and lets
    // it fall back leaves the condition exactly where it stood, and scoring
    // that as success is how a stalemate runs for a thousand intervals while
    // every line in the log reads green. Only a closer approach to the
    // threshold than any previously reached counts as the remediation having
    // done something.
    const dir = PROBE_DIRECTION[p.remediation.kind] || 1;
    let effect = 'unmeasured';
    if (probe !== null) {
      if (rec.bestProbe === null) {
        effect = 'baseline';
        rec.bestProbe = probe;
      } else if ((probe - rec.bestProbe) * dir > PROBE_EPSILON) {
        effect = 'progress';
        rec.bestProbe = probe;
        rec.stalled = 0;
        ...
        effect =
          rec.lastProbe !== null &&
          Math.abs(probe - rec.lastProbe) >= PROBE_EPSILON
            ? 'oscillating'
            : 'no-change';
        rec.stalled += 1;
      }
    ...
    const escalated = rec.stalled >= ESCALATE_AFTER;
    ...
    actions.push({
      ...base,
      status: escalated ? 'stalled' : 'applied',
      effect,
      applications: rec.applications,
      stalledFor: rec.stalled,
      probe: probe === null ? null : Number(probe.toFixed(3)),
      bestProbe: rec.bestProbe === null ? null : Number(rec.bestProbe.toFixed(3)),
      result: escalated
        ? `remediation stalled — ${label} has not improved on ${Number(
            rec.bestProbe
          ).toFixed(3)} across ${rec.stalled} applications${
            PROBE_TARGET[p.remediation.kind] !== undefined
              ? ` (threshold ${PROBE_TARGET[p.remediation.kind]})`
              : ''
          }`
        : raw,
    });
